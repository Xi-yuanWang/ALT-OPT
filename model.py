...
class GAT(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout, heads=8, output_heads=1, **kwargs):
...
class CorrectAndSmooth(torch.nn.Module):
    ...
    def correct(self, data, mlp, edge_weight=None):
        x, adj_t = data.x, data.adj_t
        label = self.init_label(data)
        mask = data.train_mask
        error = torch.zeros_like(mlp)
        error[mask] = label[mask] - mlp[mask]
        smoothed_error = self.prop1(error, adj_t, post_step=lambda z: z.clamp_(-1., 1.), edge_weight=edge_weight)
        sigma = error[mask].abs().sum() / int(mask.sum())
        scale = sigma / smoothed_error.abs().sum(dim=1, keepdim=True)
        scale[scale.isinf() | (scale > 1000)] = 1.0
        return mlp + scale * smoothed_error

    def smooth(self, data, y_soft, edge_weight=None):
        print(y_soft[0])
        x, adj_t = data.x, data.adj_t
        mask = data.train_mask
        label = self.init_label(data)
